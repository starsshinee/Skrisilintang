...
<head>
  ...
  <title>Dashboard Kepala BPMP – SIBMN</title>
  <script src="https://cdn.tailwindcss.com/3.4.17"></script>
  <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
  <script>tailwind.config={theme:{extend:{fontFamily:{jakarta:['Plus Jakarta Sans','sans-serif']},colors:{navy:{50:'#eef3ff',100:'#d9e4ff',200:'#bccfff',300:'#8eaeff',400:'#5a82ff',500:'#3355ff',600:'#1a2f9b',700:'#0f1f6e',800:'#0a1550',900:'#060d33'}}}}}</script>
  <style>
    ...
    .badge-role{background:linear-gradient(135deg,#6d28d9,#8b5cf6);}
...
<div class="flex h-full min-h-screen">
  <aside class="sidebar w-64 flex-shrink-0 flex flex-col p-5 gap-3">
    ...
    <nav class="flex-1 space-y-1">
      <a href="{{ route('kepalabpmp.dashboard') }}" class="flex items-center gap-3 text-white bg-white/15 rounded-xl px-3 py-2.5 text-sm font-semibold">
        <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>Dashboard
      </a>
      <a href="#" class="flex items-center gap-3 text-blue-200 hover:text-white hover:bg-white/10 rounded-xl px-3 py-2.5 text-sm font-medium transition">
        <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>Laporan BMN
      </a>
      <a href="#" class="flex items-center gap-3 text-blue-200 hover:text-white hover:bg-white/10 rounded-xl px-3 py-2.5 text-sm font-medium transition">
        <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 11 3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>Persetujuan
      </a>
    </nav>
    <form method="POST" action="{{ route('logout') }}">@csrf
      <button type="submit" class="w-full flex items-center gap-3 text-blue-200 hover:text-red-300 hover:bg-red-500/10 rounded-xl px-3 py-2.5 text-sm font-medium transition">
        <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" x2="9" y1="12" y2="12"/></svg>Keluar
      </button>
    </form>
  </aside>
  <main class="flex-1 overflow-auto p-8">
    <div class="flex items-center justify-between mb-8 anim">
      <div>
        <h1 class="text-2xl font-extrabold text-navy-900">Dashboard Kepala BPMP</h1>
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="w-10 h-10 rounded-full bg-purple-600 flex items-center justify-center text-white font-bold text-sm">{{ strtoupper(substr(auth()->user()->name,0,1)) }}</div>
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
      @foreach([['label'=>'Total Aset','value'=>'1.247','color'=>'bg-blue-100 text-blue-600'],['label'=>'Menunggu Persetujuan','value'=>'12','color'=>'bg-amber-100 text-amber-600'],['label'=>'Laporan Bulan Ini','value'=>'5','color'=>'bg-green-100 text-green-600']] as $s)
