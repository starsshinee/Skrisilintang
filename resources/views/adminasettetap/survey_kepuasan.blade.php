...
<title>SIPANDU - Data Survey Kepuasan</title>
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .nav-item.active { background: linear-gradient(135deg, #FEF3C7, #FDE68A); color: var(--amber); font-weight: 700; }
  ...
  .btn-filter:hover { border-color: var(--amber); color: var(--amber); }
  ...
  .status-sangat_puas { background: #FEF3C7; color: var(--amber); }
  .status-puas { background: #FEF3C7; color: var(--amber); }
  .status-cukup { background: #FEF3C7; color: var(--amber); }
  .status-kurang_puas { background: #FEF2F2; color: var(--danger); }
  .status-tidak_puas { background: #FEF2F2; color: var(--danger); }
  ...
  .action-btn:hover { background: #FEF3C7; border-color: var(--amber); transform: translateY(-1px); }
  ...
  .form-input:focus, .form-select:focus, .form-textarea:focus {
    outline: none; border-color: var(--amber); box-shadow: 0 0 0 3px rgba(245,158,11,.1);
  }
...
.btn-download svg { width: 18px; height: 18px; }
  ...
  .page-btn:hover:not(.active) { border-color: var(--amber); color: var(--amber); }
  .page-btn.active {
...
<!-- SIDEBAR - Include partials atau sesuaikan dengan struktur Anda -->
@include('partials.sidebar')

<main class="main">
  <div class="topbar">
    <span class="topbar-title">Data Survey Kepuasan</span>
  ...
  <div class="content">
    ...
    <div class="table-card">
      <div class="table-toolbar">
        <form method="GET" action="{{ route('adminasettetap.survey-kepuasan') }}" class="search-wrap">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="#94A3B8">
            <path d="M15.5 14h-.79l-.28-.27A6.47 6.47 0 0016 9.5 6.5 6.5 0 109.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/>
          </svg>
          <input type="text" name="search" placeholder="Cari nama atau email responden..." value="{{ request('search') }}">
        </form>
        <select name="kepuasan" class="filter-select" onchange="this.form.submit()">
          <option value="">Semua Rating</option>
          <option value="sangat_puas" {{ request('kepuasan') == 'sangat_puas' ? 'selected' : '' }}>Sangat Puas</option>
          <option value="puas" {{ request('kepuasan') == 'puas' ? 'selected' : '' }}>Puas</option>
          <option value="cukup" {{ request('kepuasan') == 'cukup' ? 'selected' : '' }}>Cukup</option>
          <option value="kurang_puas" {{ request('kepuasan') == 'kurang_puas' ? 'selected' : '' }}>Kurang Puas</option>
          <option value="tidak_puas" {{ request('kepuasan') == 'tidak_puas' ? 'selected' : '' }}>Tidak Puas</option>
        </select>
      </div>
      ...
      <table>
        ...
        <tbody>
          @forelse($surveys as $index => $survey)
            <tr>
              <td><strong>{{ $surveys->firstItem() + $index }}</strong></td>
              ...
              <td>
                <div class="flex items-center gap-2">
                  <span class="star-rating">
                    @php $rating = $survey->kepuasan ?? 'tidak_puas'; @endphp
                    @if($rating == 'sangat_puas') ★★★★★
                    @elseif($rating == 'puas') ★★★★
                    @elseif($rating == 'cukup') ★★★
                    @elseif($rating == 'kurang_puas') ★★
                    @else ★ @endif
                  </span>
                  <span class="status-badge status-{{ $survey->kepuasan ?? 'tidak_puas' }}">
                    {{ ucwords(str_replace('_', ' ', $survey->kepuasan ?? 'tidak puas')) }}
                  </span>
                </div>
              </td>
              <td>{{ Str::limit($survey->aspek_memuaskan ?? '-', 50) }}</td>
              <td>{{ Str::limit($survey->saran ?? '-', 50) }}</td>
              <td>
                <a href="#modal-detail-{{ $survey->id }}" class="action-btn" onclick="openModal('modal-detail-{{ $survey->id }}')" title="Detail">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="#94A3B8">
                    <path d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z"/>
                  </svg>
                </a>
                <form action="{{ route('adminasettetap.survey.destroy', $survey->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus survey ini?')">
                  @csrf
                  @method('DELETE')
            ...
            <!-- MODAL DETAIL SURVEY -->
            <div id="modal-detail-{{ $survey->id }}" class="modal-overlay">
              <div class="modal">
                <h2 class="modal-title">Detail Survey Kepuasan</h2>
                
                <div class="form-row">
                  <div class="form-group">
                    <div class="form-label">Nama Responden</div>
                    <div><strong>{{ $survey->nama ?? '-' }}</strong></div>
                  </div>
                  <div class="form-group">
                    <div class="form-label">Tanggal Survey</div>
                    <div>{{ $survey->created_at?->format('d F Y H:i') ?? '-' }}</div>
                  </div>
                </div>
                  ...
                  <div class="form-group">
                    ...
                    <div>
                      <div class="flex items-center gap-3">
                        ...
                        <span class="status-badge status-{{ $survey->kepuasan ?? 'tidak_puas' }} text-sm px-3 py-1">
                          {{ ucwords(str_replace('_', ' ', $survey->kepuasan ?? 'tidak puas')) }}
                        </span>
                      </div>
                    </div>
                ...
                <div class="form-group">
                  <div class="form-label">Aspek yang Paling Memuaskan</div>
                  <div class="form-textarea" style="border: 1px solid #E8EDF5; background: #F8FAFF; padding: 16px; min-height: 120px;">
                    {{ $survey->aspek_memuaskan ?? '-' }}
                  </div>
                </div>

                <div class="form-group">
                  <div class="form-label">Saran Perbaikan</div>
                  <div class="form-textarea" style="border: 1px solid #E8EDF5; background: #F8FAFF; padding: 16px; min-height: 120px;">
                    {{ $survey->saran ?? '-' }}
                  </div>
                </div>
                ...
                <div class="form-row">
                  ...
                  <div class="form-group">
                    ...
                    <div>{{ $survey->updated_at?->format('d F Y H:i') ?? '-' }}</div>
                  </div>
                </div>

                <div class="btn-group">
                  <button class="btn btn-secondary" onclick="closeModal('modal-detail-{{ $survey->id }}')">Tutup</button>
                </div>
              </div>
            </div>
            ...
            <tr>
              <td colspan="8" class="text-center py-8">
                <div class="text-center py-12">
                  ...
                  <h3 class="text-lg font-semibold text-gray-500 mb-2">Belum ada survey</h3>
                  <p class="text-sm text-gray-400 mb-4">Belum ada data survey kepuasan yang masuk</p>
                </div>
              </td>
            </tr>
          @endforelse
      ...
      <div class="table-footer">
        ...
        @else
          <span>Menampilkan {{ $surveys->total() }} data</span>
        @endif
