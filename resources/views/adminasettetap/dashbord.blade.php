...
<head>
  ...
  <title>Dashboard Admin Aset Tetap – SIBMN</title>
...
<div class="flex h-full min-h-screen">
  <aside class="sidebar w-64 flex-shrink-0 flex flex-col p-5 gap-3">
    ...
    <nav class="flex-1 space-y-1">
      <a href="{{ route('adminasettetap.dashboard') }}" class="flex items-center gap-3 text-white bg-white/15 rounded-xl px-3 py-2.5 text-sm font-semibold"><svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>Dashboard</a>
      <a href="#" class="flex items-center gap-3 text-blue-200 hover:text-white hover:bg-white/10 rounded-xl px-3 py-2.5 text-sm font-medium transition"><svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>Aset Tetap</a>
      <a href="#" class="flex items-center gap-3 text-blue-200 hover:text-white hover:bg-white/10 rounded-xl px-3 py-2.5 text-sm font-medium transition"><svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>Kondisi Aset</a>
      <a href="#" class="flex items-center gap-3 text-blue-200 hover:text-white hover:bg-white/10 rounded-xl px-3 py-2.5 text-sm font-medium transition"><svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>Laporan Aset</a>
    </nav>
  ...
  <main class="flex-1 overflow-auto p-8">
    <div class="flex items-center justify-between mb-8 anim">
      <div><h1 class="text-2xl font-extrabold text-navy-900">Dashboard Admin Aset Tetap</h1><p class="text-slate-500 text-sm mt-1">Selamat datang, <span class="font-semibold text-navy-700">{{ auth()->user()->name }}</span></p></div>
      <div class="flex items-center gap-3">
        <span class="badge-role text-white text-xs font-bold px-3 py-1.5 rounded-full">{{ auth()->user()->role_label }}</span>
        <div class="w-10 h-10 rounded-full bg-orange-600 flex items-center justify-center text-white font-bold text-sm">{{ strtoupper(substr(auth()->user()->name,0,1)) }}</div>
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
      @foreach([['label'=>'Total Aset Tetap','value'=>'856','color'=>'bg-orange-100 text-orange-600'],['label'=>'Kondisi Baik','value'=>'831','color'=>'bg-green-100 text-green-600'],['label'=>'Perlu Pemeliharaan','value'=>'25','color'=>'bg-red-100 text-red-600']] as $s)
