...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .form-input, .form-select, .form-textarea {
    ...
    color: var(--text-primary); background: #fff; transition: all .2s; outline: none;
  }
  ...
  .kendaraan-preview {
    ...
    border-radius: 10px; background: linear-gradient(135deg, #eff6ff, #dbeafe); border: 1px solid #bfdbfe;
  }
...
<main class="main">
  ...
  <div class="content-grid">
    ...
    <div class="form-card">
      ...
      <div class="form-body">
        <form action="{{ route('pegawai.pengembalian-kendaraan.store') }}" method="POST" enctype="multipart/form-data">
          @csrf
          ...
          <div class="form-group">
            <div class="form-label"><i class="fas fa-calendar-check"></i> Tgl & Jam Kembali <span class="req">*</span></div>
            <input type="datetime-local" name="tanggal_pengembalian_aktual" id="tglPengembalian" class="form-input" required>
          </div>
      ...
      <div class="history-card">
        <div class="history-header">
          <div class="history-title">
            ...
            <span style="font-size:14px;color:var(--text-secondary);margin-left:8px">{{ $pengembalianKendaraan->count() }} data</span>
          </div>
        ...
        <div class="req-list">
          @forelse($pengembalianKendaraan as $item)
            @php $statusClass = strtolower($item->status_pengembalian ?? 'diproses'); @endphp
            <div class="req-card" data-status="{{ $statusClass }}">
              <div class="req-card-top">
                <div style="display:flex;align-items:center;gap:12px">
                  <div class="req-card-icon"><i class="fas fa-car-side"></i></div>
                  <div>
                    <div class="req-card-name">{{ $item->peminjamanKendaraan->nama_barang ?? 'Kendaraan' }}</div>
                    <div class="req-card-code">{{ $item->peminjamanKendaraan->merek ?? '-' }}</div>
                  </div>
                </div>
    ...
    <div class="modal-box">
        ...
        <div id="contentModal" style="display:none;">
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:15px;">
                <div>
                    <label style="font-size:10px; font-weight:700; color:var(--text-secondary); text-transform:uppercase;">Kendaraan</label>
            ...
            <div style="background:#f8fafc; padding:12px; border-radius:10px; border:1px solid var(--border); margin-bottom:15px;">
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                    <div>
                        <label style="font-size:10px; font-weight:700; color:var(--text-secondary); text-transform:uppercase;">Kondisi Dilaporkan</label>
                        <div id="modKondisi" style="font-size:13px; font-weight:600;"></div>
                    </div>
                    <div>
                        <label style="font-size:10px; font-weight:700; color:var(--text-secondary); text-transform:uppercase;">Status Verifikasi</label>
            ...
            <div id="komentarAdminArea" style="display:none; margin-bottom:15px; padding:12px; background:#fef2f2; border-left:3px solid var(--danger); border-radius:6px;">
                <label style="font-size:10px; font-weight:700; color:var(--danger); text-transform:uppercase;">Tanggapan Admin / Ditolak</label>
